...
var
	...
	// 等同以下代码
	// core_contact = Array.prototype.concat,
...
jQuery.extend({
	...
	nodeName: function( elem, name ) {
		...
		// 所以统一转成小写在判断
		// 这里return elem.nodeName.toLowerCase();
		// 我认为原因是为了保持浏览器自身的对外的规则，表面所有引用nodeName都要做转换的动作
		...
		if ( args ) {
			if ( isArray ) { //数组
				for ( ; i < length; i++ ) {
					...
					// 相当于：
					// args = [arg1, arg2, arg3];
					// callback(args1, args2, args3);然后callback里面的this指向了obj[i]
		...
		// A special, fast, case for the most common use of each
			...
			if ( isArray ) {
				for ( ; i < length; i++ ) {
					...
					// 相当于callback(i, obj[i])。然后callback里面的this指向了obj[i]
					if ( value === false ) {
						break;
					}
				}
	...
	// if(String.prototype.trim &&  "\uFEFF\xA0".trim() !==''){
		// 高级的浏览器已经支持原生的String的trim
	 	// \uFEFF是utf8的字节序标记，详见：字节顺序标记 "\xA0"是全角空格
	// }
	...
	inArray: function( elem, arr, i ) {
		...
		if ( arr ) {
			...
			for ( ; i < len; i++ ) {
				...
				// jQuery这里的（i in arr）判断是为了跳过稀疏数组的元素
				// 例如 var arr = []； arr[2] = 1;
				// 此时 arr == [undefined, undefined, i]
				// 结果是 => (0 in arr == false) (1 in arr ==false)
				//但是不了解的是这里
				//测试了一下 $.inArray(undefined, arr, 0)是返回-1的
				// 也许你很不解，因为数组中明明第一个元素是undefined，后边举个例子探讨一下
